...
document.getElementById("registrationForm").addEventListener("submit", async function(event) {
    ...
    let userData = { name, phone, telegram, candidateID, examDate: formattedDate };
...
function checkRegistrationStatus() {
    ...
    if (storedUser) {
        ...
        if (now < examDate) {
            ...
            document.getElementById("examDetails").innerText = `✅ Registered!\n🆔 ${user.candidateID}\n📅 Exam Date: ${user.examDate}`;
        } else {
...
function formatExamDate(date) {
    let options = { weekday: "long", day: "2-digit", month: "2-digit", year: "2-digit" };
